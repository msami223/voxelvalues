<?php
/**
 * Default Game Header - Modern "App Style"
 * 
 * Layout:
 * [Logo + Game Switcher]   [ Center Nav (Icons) ]   [ Right Actions ]
 * 
 * Links are PRETTY: /games/{slug}/feed
 */
...
if ( is_singular('game') ) {
    $current_game_slug = $post->post_name;
    $current_game_name = get_the_title();
} elseif ( is_tax('game_ref') ) {
    $term = get_queried_object();
    $current_game_slug = $term->slug;
    $current_game_name = $term->name;
} elseif ( is_singular( array('item', 'trade') ) ) {
    $terms = get_the_terms( get_the_ID(), 'game_ref' );
    if ( $terms && ! is_wp_error( $terms ) ) {
        $current_game_slug = $terms[0]->slug;
        $current_game_name = $terms[0]->name;
    }
} else {
    $current_game_slug = 'default';
    $current_game_name = 'Select Game';
}

// 2. Define Pretty Links
// Note: We use 'games' (plural) as defined in rewrite slug
$base_url = home_url( '/games/' . $current_game_slug ); 

$home_link   = $base_url; // All Values
    ...
    <div class="app-header-container">
        ...
        <nav class="header-section center">
            ...
            <a href="<?php echo $home_link; ?>" class="nav-icon-link <?php echo (is_singular('game') && !get_query_var('voxel_subpage')) ? 'active' : ''; ?>">
                <span class="icon-box"><?php echo voxel_icon('clipboard'); ?></span>
            ...
            <a href="<?php echo $feed_link; ?>" class="nav-icon-link <?php echo (get_query_var('voxel_subpage') == 'feed') ? 'active' : ''; ?>">
                <span class="icon-box"><?php echo voxel_icon('arrows'); ?></span>
            ...
            <a href="<?php echo $create_link; ?>" class="nav-icon-link <?php echo (get_query_var('voxel_subpage') == 'create') ? 'active' : ''; ?>">
                <span class="icon-box"><?php echo voxel_icon('plus'); ?></span>
            ...
            <a href="<?php echo $my_link; ?>" class="nav-icon-link <?php echo (get_query_var('voxel_subpage') == 'my-trades') ? 'active' : ''; ?>">
                <span class="icon-box"><?php echo voxel_icon('user'); ?></span>
